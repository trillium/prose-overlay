...
- **2026-06-30 — Reverted "no-hat for non-letter tokens" decision** (the rationale above was wrong). Earlier in this session I parroted commit `3c8c9e9`'s message ("dead pixels — letter capture won't bind") and called the no-hat behavior "by design." User flagged it explicitly as a bug: digits like `123` should have a VISIBLE hat dot so you can see the token exists. Visibility (this fix) is separable from addressability (future slice). Re-added the fallback in `compute_hat_assignments` so non-letter tokens paint a hat using `token[0]`. Open follow-ups: (a) JS allocator (`js/prose_allocate_hats.js`) likely also skips digits — verify and fix the bundle so this doesn't regress whenever `hat_js_fallback=false`; (b) digit/punct addressability needs a new voice capture (e.g. `<user.prose_hat_digit>` matching 0-9) + parallel hat-to-token namespace.
