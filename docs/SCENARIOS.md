...
- **Status:** `[~]` partial — the four `pre/post <hat>` and `pre/post file` cursor-move actions now call `_recompute_hats()` before `instance.canvas.refresh()` (shipped 2026-07-01 alongside this scenario). The `take` interaction remains: `setSelection` already calls `_recompute_hats()` (`shim/actions_cursorless.py:108`), but `_recompute_hats` reads `instance.cursor` for proximity — and `take` sets `instance.buffer.set_selection(...)` without touching `instance.cursor`, so the allocator anchor doesn't follow the selection. **OQ1:** should `take` also update `instance.cursor` to the selection start/end, OR should `_recompute_hats` prefer `instance.buffer.selection` over `instance.cursor` when a selection is active? Cleaner semantics probably the latter — treat selection as the proximity anchor when present.
