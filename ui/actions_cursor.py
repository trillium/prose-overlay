...
def _set_cursor(gap, change_mode: bool = False) -> None:
    ...
    This is the ONLY place that assigns to instance.state.cursor.
    Also manages change_mode state.
    """
    instance.state.cursor = gap
    instance.state.change_mode = change_mode
...
def _prose_overlay_set_cursor(gap_index: int, change_mode: bool = False):
    ...
    instance.state.blink_on = True
    if instance.runtime.blink_job is None:
        instance.runtime.blink_job = cron.interval("500ms", _blink_tick)


def _prose_overlay_clear_cursor():
    """Internal helper: clear cursor and cancel blink job."""
    _set_cursor(None)
    instance.state.blink_on = False
    if instance.runtime.blink_job is not None:
        cron.cancel(instance.runtime.blink_job)
        instance.runtime.blink_job = None


def _blink_tick():
    """Cron callback: toggle blink state and refresh canvas."""
    instance.state.blink_on = not instance.state.blink_on
...
def _viewport_cursor_row():
    ...
    if viewport is None or not viewport._last_rows or instance.state.cursor is None:
        return viewport, None
    return viewport, viewport._find_cursor_row(viewport._last_rows, instance.state.cursor)
...
def _auto_scroll_to_cursor(strategy: str = "nearest") -> None:
    ...
    if strategy == "nearest":
        ...
        new_offset = viewport.compute_scroll_for_cursor(
            cached_rows, instance.state.cursor, viewport.get_scroll_offset(), max_vis
        )
    ...
    if align_where is None:
        return
    row = viewport._find_cursor_row(cached_rows, instance.state.cursor)
    if row is None:
        return
...
class Actions:
    ...
    def prose_overlay_cursor_end():
        ...
        _prose_overlay_set_cursor(len(instance.state.buffer))
