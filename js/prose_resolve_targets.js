...
(() => {
    ...
    "node_modules/.pnpm/lodash@4.17.21/node_modules/lodash/lodash.js"(exports, module) {
      (function() {
        ...
        var runInContext = function runInContext2(context) {
          ...
          lodash.escapeRegExp = escapeRegExp5;
          lodash.every = every;
          lodash.find = find;
  ...
  var require_sanitize_html_browser = __commonJS({
    "node_modules/.pnpm/sbd@1.0.19/node_modules/sbd/lib/sanitize-html-browser.js"(exports, module) {
  ...
  var require_Match = __commonJS({
    "node_modules/.pnpm/sbd@1.0.19/node_modules/sbd/lib/Match.js"(exports) {
      ...
      var isCapitalized = exports.isCapitalized = function(str2) {
        return /^[A-Z][a-z].*/.test(str2) || isNumber(str2);
      };
      exports.isSentenceStarter = function(str2) {
        return isCapitalized(str2) || /``|"|'/.test(str2.substring(0, 2));
      };
      exports.isCommonAbbreviation = function(str2) {
      ...
      exports.isCustomAbbreviation = function(str2) {
        if (str2.length <= 3) {
          return true;
        }
      ...
      var isNumber = exports.isNumber = function(str2, dotPos) {
        if (dotPos) {
          str2 = str2.slice(dotPos - 1, dotPos + 2);
        }
        return !isNaN(str2);
      };
      exports.isPhoneNr = function(str2) {
  ...
  var Notifier = class {
    constructor() {
       ...
       */
      __publicField(this, "notifyListeners", (...args) => {
    ...
    registerListener(listener) {
      ...
      return {
        dispose: () => {
          (0, import_lodash_es3.pull)(this.listeners, listener);
        }
      };
    }
  };
  ...
  function rangeToPlainObject(range2) {
    return {
      start: positionToPlainObject(range2.start),
      end: positionToPlainObject(range2.end)
    };
  }
  ...
  function uniqWithHash(array, isEqual2, hash) {
    ...
    needsUniq.forEach((key) => {
      hashToItems.set(key, (0, import_lodash_es7.uniqWith)(hashToItems.get(key), isEqual2));
    });
  ...
  function expandToFullLine(editor, range2) {
    return new Range(
      new Position(range2.start.line, 0),
      editor.document.lineAt(range2.end).range.end
    );
  }
  function getRangeLength(editor, range2) {
    return range2.isEmpty ? 0 : editor.document.offsetAt(range2.end) - editor.document.offsetAt(range2.start);
  }
  ...
  function union(range2, ...unionWith) {
    for (const r of unionWith) {
      if (r != null) {
        range2 = range2.union(r);
      }
    }
    return range2;
  }
  ...
  var DestinationImpl = class _DestinationImpl {
    ...
    constructEditWithDelimiters(text) {
      const range2 = this.getEditRange();
      const editText = this.getEditText(text);
      const updateRange = (range3) => {
        return this.updateRange(range3, text);
      };
      return {
        range: range2,
        text: editText,
    ...
    updateRange(range2, text) {
      const baseStartOffset = this.editor.document.offsetAt(range2.start) + this.indentationString.length + (this.insertionPrefix?.length ?? 0);
  ...
  function getIndentationString(editor, range2) {
  ...
  // packages/cursorless-engine/src/processTargets/targets/util/createContinuousRange.ts
  ...
  var BaseTarget = class _BaseTarget {
    ...
    isEqual(otherTarget) {
      return otherTarget instanceof _BaseTarget && (0, import_lodash.isEqual)(this.getEqualityParameters(), otherTarget.getEqualityParameters());
    }
    /**
  ...
  // packages/cursorless-engine/src/util/selectionUtils.ts
  function selectionWithEditorFromRange(selection, range2) {
    return selectionWithEditorFromPositions(selection, range2.start, range2.end);
  }
  ...
  function shrinkRangeToFitContent(editor, range2) {
    const { document: document2 } = editor;
    const text = document2.getText(range2);
    const startDelta = text.length - text.trimStart().length;
    const endDelta = text.length - text.trimEnd().length;
    const startOffset = document2.offsetAt(range2.start) + startDelta;
    const endOffset = document2.offsetAt(range2.end) - endDelta;
    const start = document2.positionAt(startOffset);
    const end = document2.positionAt(endOffset);
    return new Range(start, end);
  }

  // packages/cursorless-engine/src/util/tryConstructTarget.ts
  function tryConstructTarget(constructor, editor, range2, isReversed) {
    return range2 == null ? void 0 : new constructor({
      editor,
      isReversed,
      contentRange: range2
    });
  ...
  function tryConstructPlainTarget(editor, range2, isReversed) {
    return tryConstructTarget(PlainTarget, editor, range2, isReversed);
  }
  ...
  function getLeadingDelimiterRange(editor, range2) {
    const { start } = range2;
    return start.line > 0 ? new Range(editor.document.lineAt(start.line - 1).range.end, range2.start) : void 0;
  }
  function getTrailingDelimiterRange(editor, range2) {
    const { end } = range2;
    return end.line + 1 < editor.document.lineCount ? new Range(range2.end, new Position(end.line + 1, 0)) : void 0;
  }
  function constructLineTarget(editor, range2, isReversed) {
    return tryConstructTarget(LineTarget, editor, range2, isReversed);
  }

  // packages/cursorless-engine/src/processTargets/targets/NotebookCellDestination.ts
  ...
  // packages/cursorless-engine/src/processTargets/marks/getActiveSelections.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/scopeHandlers/compareTargetScopes.ts
  ...
  function partiallyContains(range1, range2, allowAdjacent) {
  ...
  var NestedScopeHandler = class extends BaseScopeHandler {
    constructor(scopeHandlerFactory2, scopeType, languageId) {
      super();
      this.scopeHandlerFactory = scopeHandlerFactory2;
  ...
  function lineNumberToScope(editor, lineNumber) {
    const { range: range2 } = editor.document.lineAt(lineNumber);
    return {
      editor,
      domain: range2,
      getTargets: (isReversed) => [createLineTarget(editor, isReversed, range2)]
    };
  }
  function createLineTarget(editor, isReversed, range2) {
    return new LineTarget({
      editor,
      isReversed,
      contentRange: fitRangeToLineContent(editor, range2)
    });
  }
  function fitRangeToLineContent(editor, range2) {
    const startLine = editor.document.lineAt(range2.start);
    const endLine = editor.document.lineAt(range2.end);
    return new Range(
  ...
  // packages/cursorless-engine/src/util/getMatchesInRange.ts
  function generateMatchesInRange(regex, editor, range2, direction) {
    const offset = editor.document.offsetAt(range2.start);
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/scopeHandlers/isPreferredOverHelper.ts
  function isPreferredOverHelper(scopeA, scopeB, matchers3) {
    ...
    for (const matcher2 of matchers3) {
  ...
  var WordScopeHandler = class extends NestedScopeHandler {
    ...
    getScopesInSearchScope({
      ...
      return contentRanges.map((range2, i) => ({
        editor,
        domain: range2,
        getTargets: (isReversed) => {
  ...
  var OneOfScopeHandler = class _OneOfScopeHandler extends BaseScopeHandler {
    ...
    static create(scopeHandlerFactory2, scopeType, languageId) {
      const scopeHandlers = scopeType.scopeTypes.map(
        (scopeType2) => {
          const handler = scopeHandlerFactory2.create(scopeType2, languageId);
          if (handler == null) {
      ...
      const iterationScopeType = () => ({
        ...
        scopeHandler: new _OneOfScopeHandler(
          void 0,
          scopeHandlers.map(
            (scopeHandler) => scopeHandlerFactory2.create(
  ...
  var ParagraphScopeHandler = class extends BaseScopeHandler {
    ...
    *generateScopeCandidates(editor, position, direction) {
      ...
      const stop = direction === "forward" ? document2.lineCount : -1;
      let startLine = getStartLine(document2, position, direction);
  ...
  function getStartLine(document2, position, direction) {
    const offset = direction === "forward" ? -1 : 1;
    const stop = direction === "forward" ? -1 : document2.lineCount;
  ...
  function createScope(editor, domain) {
    return {
      editor,
      domain,
      getTargets: (isReversed) => [
  ...
  var UrlScopeHandler = class extends RegexStageBase {
    constructor(scopeHandlerFactory2, scopeType, languageId) {
      super(scopeHandlerFactory2, scopeType, languageId);
      this.scopeType = scopeType;
  ...
  var GlyphScopeHandler = class extends RegexStageBase {
    ...
    get regex() {
      return new RegExp((0, import_lodash4.escapeRegExp)(this.scopeType.character), "gui");
    }
  };
  ...
  var ScopeHandlerFactoryImpl = class {
    constructor(languageDefinitions2) {
      this.languageDefinitions = languageDefinitions2;
  ...
  var getLineNumber = (editor, lineNumberType, lineNumber) => {
  ...
  // packages/cursorless-engine/src/util/typeUtils.ts
  ...
  var ConditionalModifierBaseStage = class {
    constructor(modifierStageFactory2, nestedModifier) {
      this.modifierStageFactory = modifierStageFactory2;
  ...
  var ModifyIfUntypedStage = class extends ConditionalModifierBaseStage {
    constructor(modifierStageFactory2, modifier) {
      super(modifierStageFactory2, modifier.modifier);
    }
    shouldModify(target) {
  ...
  var ContainingTokenIfUntypedEmptyStage = class extends ConditionalModifierBaseStage {
    constructor(modifierStageFactory2) {
      super(modifierStageFactory2, {
  ...
  var TargetPipelineRunner = class {
    constructor(modifierStageFactory2, markStageFactory) {
      this.modifierStageFactory = modifierStageFactory2;
  ...
  var TargetPipeline = class {
    constructor(modifierStageFactory2, markStageFactory, target, opts) {
      this.modifierStageFactory = modifierStageFactory2;
      this.markStageFactory = markStageFactory;
      this.target = target;
      this.opts = opts;
    ...
    processRangeTarget(targetDesc) {
      ...
      return (0, import_lodash5.zip)(anchorTargets, activeTargets).flatMap(
  ...
  function getModifierStagesFromTargetModifiers(modifierStageFactory2, targetModifiers) {
    return targetModifiers.map(modifierStageFactory2.create).reverse();
  }
  ...
  function getExcludedScope(modifierStageFactory2, target, scopeType, direction) {
    return modifierStageFactory2.create({
      type: "relativeScope",
      scopeType,
      direction,
      length: 1,
  ...
  var StoredTargetStage = class {
    constructor(storedTargets2, key) {
      this.storedTargets = storedTargets2;
      this.key = key;
    }
    run() {
  ...
  var MarkStageFactoryImpl = class {
    constructor(readableHatMap, storedTargets2) {
      this.readableHatMap = readableHatMap;
      this.storedTargets = storedTargets2;
      this.create = this.create.bind(this);
    }
  ...
  var ContainingScopeStage = class {
    constructor(modifierStageFactory2, scopeHandlerFactory2, modifier) {
      this.modifierStageFactory = modifierStageFactory2;
      this.scopeHandlerFactory = scopeHandlerFactory2;
      this.modifier = modifier;
    }
  ...
  var EveryScopeStage = class {
    ...
    getDefaultIterationRange(scopeHandler, scopeHandlerFactory2, target) {
      const iterationScopeHandler = scopeHandlerFactory2.create(
        scopeHandler.iterationScopeType,
        target.editor.document.languageId
      );
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/FilterStages.ts
  ...
  var HeadTailStage = class {
    constructor(modifierStageFactory2, isReversed, modifiers) {
      this.modifierStageFactory = modifierStageFactory2;
  ...
  var HeadStage = class extends HeadTailStage {
    constructor(modifierStageFactory2, modifier) {
      super(modifierStageFactory2, true, modifier.modifiers);
    }
  ...
  var TailStage = class extends HeadTailStage {
    constructor(modifierStageFactory2, modifier) {
      super(modifierStageFactory2, false, modifier.modifiers);
    }
    constructContentRange(originalRange, modifiedRange) {
  ...
  var InstanceStage = class {
    constructor(modifierStageFactory2, storedTargets2, modifier) {
      this.modifierStageFactory = modifierStageFactory2;
      this.storedTargets = storedTargets2;
      this.modifier = modifier;
    }
    ...
    getTargetIterable(target, editor, searchRange, direction) {
      const iterable = imap(
        generateMatchesInRange(
          new RegExp((0, import_lodash6.escapeRegExp)(target.contentText), "g"),
          editor,
          searchRange,
          direction
        ),
        (range2) => new PlainTarget({
          contentRange: range2,
          editor,
          isReversed: false,
          isToken: false
  ...
  function takeFromOffset(iterable, offset, count) {
  ...
  // packages/cursorless-engine/src/util/nodeFinders.ts
  ...
  function delimitedSelector(isDelimiterNode, defaultDelimiter, getStartNode = import_lodash7.identity, getEndNode = import_lodash7.identity) {
    return (editor, node) => {
      let leadingDelimiterRange;
      let trailingDelimiterRange;
  ...
  function getInsertionDelimiter2(editor, leadingDelimiterRange, trailingDelimiterRange, defaultDelimiterInsertion) {
    const { document: document2 } = editor;
    const delimiters = [
      trailingDelimiterRange != null ? document2.getText(trailingDelimiterRange) : defaultDelimiterInsertion,
      leadingDelimiterRange != null ? document2.getText(leadingDelimiterRange) : defaultDelimiterInsertion
    ];
    return (0, import_lodash7.maxBy)(delimiters, "length");
  }
  ...
  function cascadingMatcher(...matchers3) {
  ...
  // packages/cursorless-engine/src/util/treeSitterUtils.ts
  ...
  function functionNameBasedFinder(...names2) {
    return (node) => {
      ...
      return names2.includes(functionNode?.text) ? functionCallNode : null;
    };
  }
  function functionNameBasedMatcher(...names2) {
    return matcher(functionNameBasedFinder(...names2));
  }
  ...
  // packages/cursorless-engine/src/languages/scss.ts
  ...
  function getNodeMatcher(languageId, scopeTypeType, includeSiblings) {
    const matchers3 = languageMatchers[languageId];
    if (matchers3 == null) {
      throw new UnsupportedLanguageError(languageId);
    }
    const matcher2 = matchers3[scopeTypeType];
    if (matcher2 == null) {
      return notSupported;
    }
    if (includeSiblings) {
  ...
  var LegacyContainingSyntaxScopeStage = class {
    constructor(languageDefinitions2, modifier) {
      this.languageDefinitions = languageDefinitions2;
      this.modifier = modifier;
    }
    run(target) {
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/surroundingPair/extractSelectionFromSurroundingPairOffsets.ts
  function extractSelectionFromSurroundingPairOffsets(document2, baseOffset, surroundingPairOffsets) {
    const interior = new Range(
      document2.positionAt(baseOffset + surroundingPairOffsets.leftDelimiter.end),
      document2.positionAt(
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/surroundingPair/generateUnmatchedDelimiters.ts
  ...
  function* generateUnmatchedDelimiters(delimiterOccurrences, initialIndex, getCurrentAcceptableDelimiters, lookForward) {
    ...
    const indices = lookForward ? (0, import_lodash9.range)(initialIndex, delimiterOccurrences.length, 1) : (0, import_lodash9.range)(initialIndex, -1, -1);
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/surroundingPair/findOppositeDelimiter.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/surroundingPair/getSurroundingPairOffsets.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/surroundingPair/weaklyContains.ts
  function weaklyContains(offsets1, offsets2) {
    return offsets1.start <= offsets2.start && offsets1.end >= offsets2.end;
  }

  // packages/cursorless-engine/src/processTargets/modifiers/surroundingPair/findDelimiterPairAdjacentToSelection.ts
  ...
  function findSurroundingPairCore(scopeType, delimiterOccurrences, acceptableDelimiters, selectionOffsets, bailOnUnmatchedAdjacent = false) {
    const initialIndex = (0, import_lodash10.sortedIndexBy)(
      delimiterOccurrences,
      {
  ...
  function getIndividualDelimiters(delimiters) {
    return delimiters.flatMap((delimiter) => {
      ...
      const allDelimiterTexts = (0, import_lodash11.uniq)((0, import_lodash11.concat)(leftDelimiters, rightDelimiters));
  ...
  function findSurroundingPairParseTreeBased(editor, selection, node, delimiters, scopeType) {
    ...
    const selectionOffsets = {
      start: document2.offsetAt(selection.start),
      end: document2.offsetAt(selection.end)
    };
    const context = {
  ...
  function findSurroundingPairTextBased(editor, range2, allowableRange, delimiters, scopeType) {
    const document2 = editor.document;
    const fullRange = allowableRange ?? document2.range;
    const individualDelimiters = getIndividualDelimiters(delimiters);
    const delimiterTextToDelimiterInfoMap = Object.fromEntries(
      individualDelimiters.map((individualDelimiter) => [
        individualDelimiter.text,
    ...
    const fullRangeOffsets = {
      start: document2.offsetAt(fullRange.start),
      end: document2.offsetAt(fullRange.end)
    };
    const selectionOffsets = {
    ...
    for (let scanLength = INITIAL_SCAN_LENGTH; scanLength < MAX_SCAN_LENGTH; scanLength *= SCAN_EXPANSION_FACTOR) {
      ...
      const currentRange = new Range(
        document2.positionAt(currentRangeOffsets.start),
        document2.positionAt(currentRangeOffsets.end)
      );
      if (!currentRange.contains(range2)) {
        continue;
      }
      ...
      const pairOffsets = getDelimiterPairOffsets(
        context,
        document2.getText(currentRange),
  ...
  function getDelimiterRegex(individualDelimiters) {
    const individualDelimiterDisjunct = (0, import_lodash12.uniq)(
      individualDelimiters.map(({ text }) => text)
    ).map(import_lodash12.escapeRegExp).join("|");
  ...
  function inferDelimiterSide2(fullText, delimiterOccurrences, index, delimiter, occurrenceStartOffset) {
    const previousOccurrence = index === 0 ? null : (0, import_lodash12.findLast)(
      delimiterOccurrences,
  ...
  function processSurroundingPair(languageDefinitions2, target, scopeType) {
  ...
  function processSurroundingPairCore(languageDefinitions2, target, scopeType) {
    const { editor, contentRange: range2 } = target;
    const languageDefinition = languageDefinitions2.get(
      target.editor.document.languageId
    );
    const document2 = editor.document;
    ...
    let node;
    try {
      node = languageDefinitions2.getNodeAtLocation(document2, range2);
    ...
    const textFragmentRange = (() => {
      ...
      const textFragmentExtractor = getTextFragmentExtractor(document2.languageId);
      if (textFragmentExtractor == null) {
        return document2.range;
      }
      const selectionWithEditor = {
        editor,
        selection: new Selection(range2.start, range2.end)
      };
  ...
  function getIterationScope(languageDefinitions2, target) {
    let surroundingTarget = getSurroundingPair(languageDefinitions2, target);
  ...
  function useInteriorOfSurroundingTarget(languageDefinitions2, target, surroundingTarget) {
  ...
  function getParentSurroundingPair(languageDefinitions2, editor, target) {
  ...
  function getSurroundingPair(languageDefinitions2, target) {
    return processSurroundingPair(languageDefinitions2, target, {
      type: "surroundingPair",
      delimiter: "collectionBoundary",
      requireStrongContainment: true
    });
  }
  function getStringSurroundingPair(languageDefinitions2, target) {
    return processSurroundingPair(languageDefinitions2, target, {
      type: "surroundingPair",
      delimiter: "string",
      requireStrongContainment: true
  ...
  var ItemStage = class {
    ...
    getEveryTarget(languageDefinitions2, target) {
    ...
    getSingleTarget(languageDefinitions2, target) {
      const itemInfos = getItemInfosForIterationScope(
        languageDefinitions2,
        target
      );
  ...
  function getItemInfosForIterationScope(languageDefinitions2, target) {
    const { range: range2, boundary } = getIterationScope(languageDefinitions2, target);
    return getItemsInRange(target.editor, range2, boundary);
  }
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/LeadingTrailingStages.ts
  ...
  function getEveryScopeTargets(modifierStageFactory2, target, scopeType) {
    const containingStage = modifierStageFactory2.create({
      type: "everyScope",
      scopeType
    });
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/RangeModifierStage.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/RawSelectionStage.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/TooFewScopesError.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/relativeScopeLegacy.ts
  function runLegacy(modifierStageFactory2, modifier, target) {
    const targets = getEveryScopeTargets(
      modifierStageFactory2,
  ...
  function computeProximalIndex(modifier, inputTargetRange, targets, isForward, containingIndices) {
    ...
    if (containingIndices == null) {
      ...
      ) : (0, import_lodash13.findLastIndex)(
        targets,
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/SurroundingPairStage.ts
  ...
  function processedSurroundingPairTarget(languageDefinitions2, modifier, target) {
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/VisibleStage.ts
  ...
  // packages/cursorless-engine/src/processTargets/modifiers/scopeTypeStages/BoundedNonWhitespaceStage.ts
  var BoundedNonWhitespaceSequenceStage = class {
    constructor(languageDefinitions2, modifierStageFactory2, modifier) {
      this.languageDefinitions = languageDefinitions2;
      this.modifierStageFactory = modifierStageFactory2;
      this.modifier = modifier;
    }
  ...
  var ModifierStageFactoryImpl = class {
    constructor(languageDefinitions2, storedTargets2, scopeHandlerFactory2) {
      this.languageDefinitions = languageDefinitions2;
      this.storedTargets = storedTargets2;
      this.scopeHandlerFactory = scopeHandlerFactory2;
      this.create = this.create.bind(this);
  ...
  // packages/cursorless-engine/src/actions/proseTargetsStandalone.ts
  var FAKE_IDE = {
    configuration: {
      getOwnConfiguration: (key) => key === "wordSeparators" ? ["_"] : void 0,
      onDidChangeConfiguration: () => ({ dispose: () => {
      } })
    },
    capabilities: { commands: {} },
    clipboard: void 0,
    messages: { showMessage: () => Promise.resolve(void 0) },
    activeTextEditor: void 0
  };
  injectIde(FAKE_IDE);
  var UnsupportedLanguageError2 = class extends Error {
    constructor() {
      super("Prose overlay has no parse tree");
      this.name = "UnsupportedLanguageError";
    }
  };
  var languageDefinitions = {
    get: () => void 0,
    getNodeAtLocation: () => {
      throw new UnsupportedLanguageError2();
    }
  };
  var storedTargets = new StoredTargetMap();
  var scopeHandlerFactory = new ScopeHandlerFactoryImpl(languageDefinitions);
  var modifierStageFactory = new ModifierStageFactoryImpl(
    languageDefinitions,
    storedTargets,
    scopeHandlerFactory
  );
  var ProseTargetsDocument = class {
    constructor(text) {
      this.text = text;
      this.uri = {
        toString: () => "prose://overlay",
        scheme: "prose",
        path: "/overlay",
        authority: "",
        query: "",
        ...
        fsPath: "/overlay",
        with: () => this.uri,
        toJSON: () => ({ scheme: "prose", path: "/overlay" })
      };
      this.languageId = "plaintext";
      this.version = 1;
      this.lineCount = 1;
      this.eol = 1;
      this.range = new Range(0, 0, 0, text.length);
    }
    offsetAt(pos) {
      return Math.min(Math.max(0, pos.character), this.text.length);
    }
    positionAt(offset) {
      return new Position(0, Math.min(Math.max(0, offset), this.text.length));
    }
    getText(range2) {
      if (range2 == null)
        return this.text;
      return this.text.slice(this.offsetAt(range2.start), this.offsetAt(range2.end));
    }
    lineAt(_lineOrPos) {
      return {
        text: this.text,
        lineNumber: 0,
        range: new Range(0, 0, 0, this.text.length),
        rangeIncludingLineBreak: new Range(0, 0, 0, this.text.length),
        firstNonWhitespaceCharacterIndex: 0,
        lastNonWhitespaceCharacterIndex: this.text.length,
        isEmptyOrWhitespace: this.text.trim().length === 0
      };
    }
  };
  var ProseTargetsEditor = class {
    constructor(text, initialSelection) {
      this.id = "prose-overlay-editor-0";
      this.isActive = true;
      this.options = {};
      this.document = new ProseTargetsDocument(text);
      this.selections = [initialSelection];
      this.visibleRanges = [new Range(0, 0, 0, text.length)];
    }
    isEqual(other) {
      return other.id === this.id;
    }
  };
  var ProseHatMap = class {
    constructor(json2, editor) {
      this.tokens = /* @__PURE__ */ new Map();
      for (const entry of json2.entries) {
        const range2 = new Range(0, entry.startCol, 0, entry.endCol);
        const token = {
          editor,
          range: range2,
          text: entry.text,
          offsets: { start: entry.startCol, end: entry.endCol }
        };
        this.tokens.set(`${entry.color}:${entry.grapheme}`, token);
      }
    }
    getToken(hatStyle, character) {
      const key = `${hatStyle}:${character}`;
      const tok = this.tokens.get(key);
      if (!tok) {
        throw new Error(`No token for hat '${hatStyle}' '${character}'`);
      }
      return tok;
    }
    getEntries() {
  ...
  function proseResolveTarget(targetJson, documentJson, hatMapJson, _cursorJson) {
    try {
      const target = JSON.parse(targetJson);
      const doc = JSON.parse(documentJson);
      const hatMap = JSON.parse(hatMapJson);
      const anchor = new Position(0, doc.cursorAnchorChar);
      const active = new Position(0, doc.cursorActiveChar);
      const sel = new Selection(anchor, active);
      const editor = new ProseTargetsEditor(doc.text, sel);
      const hats = new ProseHatMap(hatMap, editor);
      FAKE_IDE.activeTextEditor = editor;
      const markStageFactory = new MarkStageFactoryImpl(hats, storedTargets);
      const runner = new TargetPipelineRunner(
        modifierStageFactory,
        markStageFactory
      );
      markStageFactory.setPipelineRunner(runner);
      const targets = runner.run(target);
      if (targets.length === 0) {
        return JSON.stringify({ error: "no targets produced" });
      }
      const contentRanges = targets.map((t) => ({
        start: { line: t.contentRange.start.line, character: t.contentRange.start.character },
        end: { line: t.contentRange.end.line, character: t.contentRange.end.character },
        isReversed: t.isReversed
      }));
      return JSON.stringify({ contentRanges });
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      return JSON.stringify({ error: msg });
    } finally {
      FAKE_IDE.activeTextEditor = void 0;
    }
  }
  globalThis.proseResolveTarget = proseResolveTarget;
})();
