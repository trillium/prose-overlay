...
# already in place. Only populates when instance.state.history is empty (which
...
def _load_history_on_startup() -> None:
    if instance.state.history:
        return
    persisted = _load_history()
    if persisted:
        instance.state.history.extend(persisted)
...
def _on_draw_history(c, overlay):
    ...
    rect = draw_mod.draw_history_panel(c, overlay, instance.state.history, instance.state.history_page)
    if rect:
...
def _on_history_overlay_hide():
    ...
    action from here. That action calls instance.runtime.history_overlay.hide(),
    ...
    """
    instance.runtime.ctx_history.tags = []
...
class Actions:
    def prose_overlay_add_text(text: str):
        ...
        instance.state._last_input_source = "text"
        if instance.state.cursor is not None:
            words = text.strip().split()
            instance.state.buffer.insert_at(instance.state.cursor, text)
            _set_cursor(instance.state.cursor + len(words), False)
            _recompute_hats()
            _auto_scroll_to_cursor()
            instance.runtime.canvas.refresh()
        else:
            instance.state.buffer.add_text(text)
            _recompute_hats()
            _auto_scroll_to_cursor()
            instance.runtime.canvas.refresh()
    ...
    def prose_overlay_add_chars(chars: str):
        ...
        extending = (
            instance.state.cursor is None
            and bool(instance.state.buffer.get_tokens())
        )
        if extending:
            tokens = instance.state.buffer.get_tokens()
            new_tokens = tokens[:-1] + [tokens[-1] + chars]
            instance.state.buffer.commit_start("extend_chars", EditKind.STRUCTURAL)
        ...
        instance.state._last_input_source = "chars"
    ...
    def prose_overlay_toggle_history():
        ...
        if instance.runtime.history_overlay.is_showing:
            actions.user.prose_overlay_hide_history()
        else:
            instance.state.history_page = 0
            instance.runtime.history_overlay.show()
            instance.runtime.ctx_history.tags = ["user.prose_history_active"]
    ...
    def prose_overlay_history_next():
        ...
        draw_mod = instance.runtime.draw_mod
        total_pages = max(1, (len(instance.state.history) + draw_mod.HISTORY_PAGE_SIZE - 1) // draw_mod.HISTORY_PAGE_SIZE)
        instance.state.history_page = min(instance.state.history_page + 1, total_pages - 1)
        instance.runtime.history_overlay.freeze()

    def prose_overlay_history_back():
        """Go to the previous history page."""
        instance.state.history_page = max(0, instance.state.history_page - 1)
        instance.runtime.history_overlay.freeze()
    ...
    def prose_overlay_history_pick(n: int):
        ...
        n_hist = len(instance.state.history)
        if not (1 <= n <= n_hist):
            print(
            ...
            return
        entry = instance.state.history[n - 1]
        print(
    ...
    def prose_overlay_history_paste(n: int):
        ...
        """
        if not (1 <= n <= len(instance.state.history)):
            return
        entry = instance.state.history[n - 1]
        actions.user.prose_overlay_hide_history()
        ...
        if instance.state.target_recall_name:
            actions.user.recall_window(instance.state.target_recall_name)
            actions.sleep("80ms")
    ...
    def prose_overlay_confirm():
        ...
        instance.state.history.insert(0, text)
        if len(instance.state.history) > _HISTORY_MAX:
        ...
        _save_history(instance.state.history)
